...
import reportFakerFactory from "./reportFakerFactory";
...
const ReportFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = reportFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("report").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ReportFakerDialogComponent);
