...
const EmployeeCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            ...
            employeeType: _entity.employeeType,
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="employee-create-dialog-component">
                ...
                <div>
                    <p className="m-0" >employeeType:</p>
                    <InputText className="w-full mb-3" value={_entity?.employeeType} onChange={(e) => setValByKey("employeeType", e.target.value)}  />
                </div>
                <div>
                    <p className="m-0" >IP Address:</p>
                    <InputText className="w-full mb-3" value={_entity?.ipAddress} onChange={(e) => setValByKey("ipAddress", e.target.value)}  />
                </div>
